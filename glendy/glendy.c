...
allocimages(void)
{
	...
	// Port comment
	//
	// Dynamic face directory path value
	#ifndef FACE_DIR
		#define FACE_DIR "/lib/face"
	#endif
	gl = eloadfile(FACE_DIR "/48x48x4/g/glenda.1");
...
resize(void)
{
	// Port comment
	//
	// Removed useless variable 'fd' declaration
	int size = (Dx(screen->r) > Dy(screen->r)) ? Dy(screen->r) + 20 : Dx(screen->r)+20; 
...
main(int argc, char **argv)
{
	...
	// Port comment
	//
	//The USED macro only takes one argument
	USED(argc);
	USED(argv);
