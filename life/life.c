...
generate(void)
{
	...
	for (i = 1; i != NLIFE - 1; i++)
		if (interest(row, i)) {
			for (j = jj0, lp = &life[i][jj0]; j <= jj1; j++, lp++)
				switch (action[*lp]) {
				case 'b':
					++*lp;
					++drow[i];
...
centerlife(void)
{
	...
	if (i0 + di < 1)
		...
		di = NLIFE - 2 - i1;
	dj = NLIFE / 2 - (jj0 + jj1) / 2;
	if (jj0 + dj < 1)
		dj = 1 - jj0;
	else if (jj1 + dj >= NLIFE - 1)
		dj = NLIFE - 2 - jj1;
	if (di != 0 || dj != 0) {
		if (di > 0) {
			iinc = -1;
		...
		for (i = i0; i * iinc <= i1 * iinc; i += iinc)
			for (j = jj0; j * jinc <= jj1 * jinc; j += jinc)
				if (life[i][j] & 1) {
					birth(i + di, j + dj);
					death(i, j);
				}
...
window(void)
{
	...
	for (i1 = NLIFE - 2; i1 != i0 && row[i1] == 0; --i1)
		;
	for (jj0 = 1; jj0 != NLIFE - 2 && col[jj0] == 0; jj0++)
		;
	for (jj1 = NLIFE - 2; jj1 != jj0 && col[jj1] == 0; --jj1)
		;
}
