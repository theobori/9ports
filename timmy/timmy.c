...
// Port comment
//
// In this file, most of the patches are for mousectl struct instances
// that try to bypass the plan9port `m` (Mouse) member.
...
mouse(void)
{
	...
	mc->m.xy = subpt(mc->m.xy, screen->r.min);

	if(lbut < 0)
		lbut = mc->m.buttons;
	if(ptinrect(mc->m.xy, work->r)){
		p = subpt(mc->m.xy, work->r.min);
	...
	if((~mc->m.buttons & lbut & 1) != 0){
		if(ptinrect(mc->m.xy, tray->r)){
			carry = objclick(subpt(mc->m.xy, tray->r.min), &trayo);
			if(carry != nil)
				carry = objdup(carry);
		}else if(ptinrect(mc->m.xy, work->r)){
			if(carry != nil)
				place();
			else{
				carry = objclick(subpt(mc->m.xy, work->r.min), &worko);
				if(carry != nil)
					objexcise(carry);
			}
		}
	}
	if((~mc->m.buttons & lbut & 4) != 0){
		if(carry != nil){
			freeobj(carry);
			carry = nil;
			showcarry = 0;
